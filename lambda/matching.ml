  ...
  | _ ->
      ...
      let lambda1 =
        ...
        | _ -> (
            ...
            | n, 0, { any_const = None; nonconsts = []; any_nonconst = None } ->
            ...
            | n, _, _ -> (
                let act0 =
                  ...
                  match (fail_opt, cases.nonconsts, cases.any_nonconst) with
                  | Some a, [], None
                  | _, _, Some a -> Some a
                  | Some _, nonconsts, None ->
                      if List.length nonconsts = cstr.cstr_nonconsts then
                        same_actions nonconsts
                      else
                        None
                  | None, nonconsts, None -> same_actions nonconsts
                in
                ...
                | None ->
                    (* Emit a switch, as bytecode implements this sophisticated
                      instruction *)
                    let fail_opt =
                      match fail_opt, cases.any_const, cases.any_nonconst with
                      | None, None, None -> None
                      | Some a, None, None
                      | None, Some a, None
                      | None, None, Some a ->
                          Some a
                      | _, Some const_act, Some nonconst_act
                      | Some const_act, None, Some nonconst_act
                      | Some nonconst_act, Some const_act, None ->
                          Some (test_isint const_act nonconst_act)
                    in
                    let sw =
